...
# -*- coding:utf-8 -*-
############################
#File Name: emailcli.py
#Author: ykyk
#Mail: 525178992@qq.com
#Created Time: 2019-01-06 12:38:39
############################
...
def get_argparse():
    parser = argparse.ArgumentParser(description='An email client in terminal')
    ...
    parser.add_argument('-r', action='store', nargs='*', dest='recipients', required=True, help='recipient who you are sendgin the email to')
...
def send_mail(meta):
    ...
    with yagmail.SMTP(user = meta.username, password = meta.password,
        ...
        logger.info('ready to send email "{0} to {1}"'.format(meta.subject,
                                                              meta.recipients))
...
def main():
    ...
    if meta.attaches:
        for attaches in meta.attaches:
            if not os.path.exists(attach):
                logger.eror('{0} is not exists'.format(attach))
                raise SystemExit()
