...
mem_total=$(free -m | awk '/Mem:/{ print $4 + $6 + $7'})
mem_free=$(free -m | awk '/Mem:/{ print $2'})
mem_used=$(free -m | echo "$mem_total - $mem_free" | bc)
mem_rate=$(free -m | echo "$mem_used * 100 / $mem_total" | bc )
